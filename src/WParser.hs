...
newtype WLabel = WLabel [Token] deriving (Eq, Show)

type Command = WInstruction WLabel
...
labelP = WLabel <$> (some (token Space <|> token Tab) <* token LF)
...
commandToTokens c = case c of
  ...
  -- Flow
  Label (WLabel l) -> [LF, Space, Space] ++ l ++ [LF]
  Call (WLabel l) -> [LF, Space, Tab] ++ l ++ [LF]
  Branch Any (WLabel l) -> [LF, Space, LF] ++ l ++ [LF]
  Branch Zero (WLabel l) -> [LF, Tab, Space] ++ l ++ [LF]
  Branch Neg (WLabel l) -> [LF, Tab, Tab] ++ l ++ [LF]
  Return -> [LF, Tab, LF]
  End -> [LF, LF, LF]
  -- Heap
  Store -> [Tab, Tab, Space]
...
instance Arbitrary WLabel where
  ...
  arbitrary = WLabel . filter (/= LF) <$> arbitrary
